...
class ItemController extends AbstractController
{
    ...
    /**
     * @Rest\Post("/user/items/todolist/{id}")
     * @param Request $request
     */
    public function postItemeTodolist(int $id,ItemRepository $itemRepository, ItemService $itemService,Request $request,ToDoListServiceRepository $toDoListServiceRepository,MailerInterface $mailer,UserRepository $userRepository )
    {
        $body = json_decode($request->getContent(), true);
        $response = new JsonResponse();
        if($body){
            $list = $toDoListServiceRepository->find($id);
            $list->setMailer($mailer);
            // dd($list);
            $user = $userRepository->find($body['user_id']);
